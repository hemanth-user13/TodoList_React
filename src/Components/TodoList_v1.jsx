...
function TodoList() {
  ...
  // Function to determine the text color based on priority
            ...
            {tasks.map((item) => (
              <li key={item.id} className="mt-2">
                <div className="flex align-middle justify-center text-2xl">
                  <span className="font-semibold text-center text-green-600">Task:</span>
                  <p className="ml-8">{item.task}{" "}</p>
                  <span className={`font-semibold ml-4 ${getPriorityColor(item.priority)}`}>
